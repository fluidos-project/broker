...
for user in "${!users_array[@]}"; do  
  ...
  if [ ${users_array[$user]} = "localAgent" ]; then
    echo "setting localAgent permissions"
    rabbitmqctl set_user_tags localAgent administrator
    rabbitmqctl set_permissions -p / localAgent ".*" ".*" ".*"
  else
